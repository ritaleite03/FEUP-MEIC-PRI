...
def hybrid_search(endpoint, collection, query, weight_semantic, weight_lexical):
    ...
    collection = "diseases"
    result_lexical = solr_lexical_query(endpoint, collection, query)
    ...
    for result in result_lexical:
       ...
       result["vector"] = result.get('vector')
    ...
    sorted_results = sorted(merge_result, key=lambda x: x.get('score'), reverse=True)
    ...
    for doc in sorted_results:
        response['response']['docs'].append({'id': doc.get('id'), 'Name': doc.get('Name'), 'score': doc.get('score'), 'vector': doc.get('vector')})
        i+=1
        if i == 20:
            break
        # print(f"* {doc.get('id')} {doc.get('Name')} [score: {doc.get('score'):.2f}]")
...
if __name__ == "__main__":
    ...
    response = hybrid_search(solr_endpoint, collection, args.query, weight_semantic, weight_lexical)
