    ...
    if table:
        ...
        for row in rows:
            if row.find('th') and row.find('th').text == "Other names": 
                ...
                info = get_row_info(row)
                disease_info["Risk factors"] = info
...
with open('teste_TUVWXYZ.json', 'w', encoding='UTF-8') as file:
